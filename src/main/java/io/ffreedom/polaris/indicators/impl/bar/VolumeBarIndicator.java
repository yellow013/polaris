		...
		// 否则则此次行情需要写入多个节点
			...
			while (unhandledVolume > limitVolume) {
				...
				if (marketDataLocalDatetime.equals(currentPointDatetime))
					...
					newBar = VolumeBar.with(currentPoint.getIndex() + 1, instrument, marketDataLocalDatetime,
							limitVolume);
				// 将新创建的节点加入节点集合
				pointSet.add(newBar);
				// 新节点写入价格,处理的数量是最大限制数量
				newBar.handleData(lastPrice, limitVolume);
				// 更新节点的上一次行情
				newBar.updatePreMarketData(marketData);
				// 剩余计算量减去单个Bar限制量
				unhandledVolume -= limitVolume;
			}
			// 剩余的未计算量小于单个Bar的最大写入量
