...
public class CandleChart extends AbstractIndicator<Candle> {
	...
	protected void initPoints() {
		...
		timeTwinSet.each(timeTwin -> points.add(Candle.withTimeTwin(timeTwin, instrument, period)));
	}

	@Override
	protected Candle initCurrentPoint() {
		return points.first();
	}
	...
	public void onMarketData(BasicMarketData marketData) {
		if (isCurrentCandlePeriod(marketData)) {
			...
			Optional<Candle> nextCandle = points.getNext(currentPoint);
			if (nextCandle.isPresent()) {
				...
				currentPoint = Candle.withTimeTwin(
						TimeTwin.of(TradingDayKeeper.get(instrument), newStartTime, newEndTime), instrument, period);
